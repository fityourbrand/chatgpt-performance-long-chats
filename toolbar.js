;(() => {
  ...
  function update() {
    ...
    if (ui.modeBtn) {
      ...
      const subtitle =
        mode.key === 'off'
          ? 'Initial fetch limiting is disabled. Other optimizations stay active.\n\nClick to return to Balanced mode.'
          : `${mode.subtitle}\nCurrently loading approximately ${Math.round(mode.loadLimit / 2)} messages for context.\n\nClick to switch mode.\nShift-click to disable initial fetch limiting.`
    ...
    modeBtn.addEventListener('click', async (e) => {
      ...
      if (isShiftClick) {
        if (CPLC.state.settings.loadMode !== 'off') {
          ...
          CPLC.state.settings.loadLimit = null
        } else {
